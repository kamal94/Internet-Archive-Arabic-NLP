class iavw:
	...
	def getIDsFromFile(self, filename: str):
	...
	def downloadMetaFiles(self, IDs: list, directoryToSave: str):
	...
	def getMetaFromFile(self, fileDir: str):
	...
	def readMetaTextInDirectory(self, directory: str):
		"""Reads all the metadata files ending in .txt
		in the passed directory"""
		import os
		return [self.getMetaFromFile(directory + os.sep + f) 
				for f in os.listdir(directory)
				if  f.endswith('.txt')]

	def proccessText(self, text: str):
	...
	def metadataToVWline(self, metadata: dict, positive: bool):
			...
			for key in metadata:
				if key in ignored_keys:
					...
					if(type(metadata[key]) == list):
						...
						data += key + ' ' + self.proccessText(string) + " |"
					else:
						data += key + ' ' + self.proccessText(metadata[key]) + " |"
	...
	def writeVWlinesToFile(self, vwLines: list, filename: str):
